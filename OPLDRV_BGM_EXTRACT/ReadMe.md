...
| オプション           | 機能                                             |
|----------------------|--------------------------------------------------|
| ```/l:ファイル名```  | 解析ログをテキストファイルに出力
| ```/o:ファイル名```  | バイナリファイルを出力（/bか/-bで形式指定可能）
| ```/b```             | BSAVE形式で出力（省略時は入力ファイルと同じ形式）
| ```/-b```            | RAW形式(oplファイルなど/BSAVEヘッダ無し)で出力（省略時は入力ファイルと同じ形式）
| ```/a:アドレス```    | (RAW形式読み込み時)バイナリ開始アドレスを指定（16進数4桁）
| ```/a:アドレス```    | 音楽データ開始アドレスを別途指定（16進数4桁）... 先頭が楽曲データとは限らない/複数曲が入っているバイナリに対応
...
- MMLの出力  
  一応仮実装。  
  詳細はOPLDRV_tool.exeを実行で出るヘルプ参照。  
  テンポ指定はログ出力のtickとtempo_list 参照で。  
  https://github.com/uniskie/msx_music_data/blob/master/doc/tempo_list.md

### 本来作ろうとしていた機能

- MuSICAバイナリファイルからOPLDRVファイルへの変換

(MuSCIA2OPLDRVは実装難易度が低いように思われるが、FMBIOS OPLDRV対応のコンパイラは多数存既存の物がある)
