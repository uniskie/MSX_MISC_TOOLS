...
int main(int argc, char* argv[])
{
	...
	u16 in_base_address = 0;			//!< バイナリのMSX上ベースアドレス
	u16 in_music_address = 0xffff;			//!< 演奏データのMSX上ベースアドレス
    ...
    while (argi < argc)
    {
        ...
        if (arg.size())
        {
			...
			//-- arg: バイナリデータのベースアドレス指定（RAW+ユーザー定義音色用）
			if (l.substr(0, 3) == "/a:")
			{
			...
			//-- arg: 演奏データのベースアドレス指定（複数アドレスまたは、演奏データが先頭にないバイナリ用）
			if (l.substr(0, 3) == "/s:")
			{
	...
	if (in_music_address == 0xffff) // 未設定の場合 ベースアドレス=音楽データ先頭
	{
