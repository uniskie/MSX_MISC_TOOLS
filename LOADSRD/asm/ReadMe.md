...
|エントリ|名前|値範囲|説明
|---|---|---|---|
|SR(0,N)| Y座標 | -32768～32767 | -31～211または195の範囲の時だけ表示。216を指定しても217に自動補正する（低優先度一括非表示を回避するため）
|SR(1,N)| X座標 | -32768～32767 | -31～255の範囲の時だけ表示。マイナスの時は画面左から少しずつ見えるように補正表示(自動でECビットを使用)
|SR(2,N)| パターン番号 | 0～63 | 16x16スプライト専用のため。PUT SPRITEのパターン番号と同じ仕様。
|SR(3,N)| 特殊フラグ | -1、0～31 | -1なら非表示。<br> ビット3～0がアニメーションウェイト。1以上ならアニメーション。<br>ビット4が0なら2パターンアニメ。1なら4パターンアニメ<br>例）<br>   0 : アニメ無し<br>   1 : メインループで毎回アニメ（2パターン）<br>  &H14 : メインループ4回毎にアニメ（4パターン）
