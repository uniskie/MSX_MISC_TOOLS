...
- 逆アセンブルリスト出力
  - `Ctrl + Enter`で選択範囲を逆アセンブルしてDisAssembleViewウィンドウに出力
  ...
  - *DisAssembleView* ウィンドウの操作
  
    ![](disasm.png)  
    
    - 'G'：カーソル位置に`Xxxxx`形式のジャンプラベルがあれば該当位置へジャンプ  
      （※ `CALL`/`JP`/`JR`/`DJNZ`やアドレス指定で、かつ、アセンブルリスト内にあるアドレスが対象）
    - `B`：ジャンプ履歴：一つ前の位置へ移動
    - `F`：ジャンプ履歴：一つ次の位置へ移動
