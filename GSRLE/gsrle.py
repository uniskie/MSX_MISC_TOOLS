...
def get_pal_table(screen_no: int):
    ...
    if ((s<-1) or (s>=len(PAL_TABLE_LIST))):
        return -1
    return PAL_TABLE_LIST[screen_no+1]
...
type_id, start_address, end_address, run_address = struct.unpack('<BHHH', data[0:7])
print('--- source file ---')
...
## RLE encode
compressed = rleEncode(data[7:7+use_size])
...
if (use_compressed):
    outdata[7:]=compressed
else:
    outdata[7:]=data[7:7+use_size]
