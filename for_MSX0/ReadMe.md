...
MSX0 Iot ROM ver 0.05.04 から ver 0.07.08でI/Oポートが変更されました。
ポートの変更以外はver 0.05.04の実装に合わせた内容です。
独自調査なので一部間違いがある可能性があります。

また、非公式の内容なので、今後仕様変更される可能性があります。
...
## 利用について
画像データ(SC5,edg,png)以外のファイルは改変・再配布自由です。  
ライセンス表示も製作者表示も不要です。  
ただし、これらのファイルの使用については当方では一切の責任を負いません。
...
[BB2.BAS](BIGAJUU/BB.BAS) ... べーしっ君で重力加速度センサー対応、重力加速度センサーが無ければランダムで目が変化。(トリガ2かENTERでモード変変更、トリガ1やタッチでセンサーリセット）

[BB.BAS](BIGAJUU/BB.BAS) ... べーしっ君で重力加速度センサー対応

[B.BAS](BIGAJUU/BB.BAS) ... べーしっ君を使用しない通常の重力加速度センサー対応

[BIGAJUU.BAS](BIGAJUU/BIGAJUU.BAS) ... 重力加速度センサー無し
...
- MPU6886(9軸重力加速度センサー)の利用
  - タッチまたはトリガで位置リセット
  - 位置リセット時、Y軸の範囲外の時はZ軸に切り替え（逆も同様）
- べーしっ君存在チェック  
  あれば利用する  
  ない場合でも動作
- MSX0 IOT、MPU6886(9軸重力加速度センサー)の存在チェック  
  あれば利用する  
  ない場合でも動作
- タッチパネルや傾きで目が動く
- 時計機能
...
- デバイスID
  ...
  - device/i2c_i/68 ... [Bottom2] MPU6886(9軸重力加速度センサー)
...
## MSX0 I/Oポート
- PORT &H58 : IOT操作（入出力）
- PORT &H57: ターミナルコンソールへの出力（出力）

> MSX0 Iot ROM ver 0.05.04 から ver 0.07.08でI/Oポートが変更  
> &H08→&H58  
> &H10→&H57
