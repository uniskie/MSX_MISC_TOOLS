...
> 非公式の内容なので、今後予告なく仕様変更される可能性があります。

> [!Note]
>  2024/09/25  
> バイト長2以上を指定した文字列読み書きについて、  
> 読み書き先のI2Cアドレスが自動加算されると教えて頂いたので情報追加しました。  
> ※ ```IOTCMS.ASM``` のバーストアクセス機能は無駄な処理になるので、廃止して作り直す予定です

> [!WARNING]
> 現在、IOTCMD.BINを使ったサンプルは暫く経つとMSX0本体ごとフリーズするようになっています。  
> マシン切り替えもリセット要求も反応しなくなって原電を切るしかなくなります。  
> ですので、このページの内容はBIGAJUUだけ参考にしてくださいませ。
...
> [!NOTE]  
> 1. データ長が2以上の連続書き込み時は、1バイトごとに書き込み先I2Cレジスタアドレスが加算される
> 2. 多バイト数値を文字列型として書き込む場合、ビッグエンディアンを要求される事が多い（各デバイス仕様書参照の事）
>
...
> [!NOTE]  
> 1. データ長が2以上の連続読み込み時は、1バイトごとに読み込み元I2Cレジスタアドレスが加算される
> 2. 多バイト数値を文字列型として取得した場合、ビッグエンディアンになっている事が多い（各デバイス仕様書参照の事）

> [!CAUTION]
>  (未検証)   
> ※ データ長が64byte以上の場合は分割が必要かも？  
> 
> ```
> #80を出力 ;データ取得の開始
> データ長を取得
