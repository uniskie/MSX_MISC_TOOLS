...
|ツール|説明|
|---|---|
| [macro/MSXBASIC.js](macro/MSXBASIC.js) | サクラエディタやEmEditorで開いたMSX BASICファイルをアスキーリスト形式に変換するマクロ<br><br>エディタ上でSJISやUTF-8での保存処理を経由するとかな文字などが**文字化け**してしまうので、エディタ上で変換するだけでなく、`元ファイル名.ASC` でアスキーリスト変換後のファイルも作成します。<br>**（同じファイル名があっても強制上書きするので注意してください）**<br><br>使い方：それぞれのエディタのmacroフォルダに置き、メニューから選択して使用します。 |
